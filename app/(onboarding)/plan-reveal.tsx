...
export default function PlanRevealScreen() {
  ...
  const vertical: Vertical = oneParam(params.vertical) === "health" ? "health" : "home";
  const assetId = oneParam(params.assetId);
  const assetName = oneParam(params.assetName) || (vertical === "home" ? "home" : "your loved one");
  ...
  const { data } = useQuery<{ items: RevealItem[]; count: number }>({
    queryKey: ["onboarding_reveal", vertical, assetId],
    queryFn: async () => {
      if (!assetId) return { items: [], count: 0 };
      if (vertical === "home") {
      ...
      const items = (rowsRes.data ?? [])
        ...
        .map((r) => ({
          ...
          dueLabel: formatDue(
            r.next_due_date,
            r.interval_months != null ? `Every ${r.interval_months} months` : null,
          ),
          costStr: formatCost(r.estimated_cost),
        }));
      return { items, count: countRes.count ?? 0 };
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 24 }]} showsVerticalScrollIndicator={false}>
        ...
        {!hasItems && (
          <View style={styles.header}>
            <Text style={styles.title}>{pollTimedOut ? `${assetName} is saved` : "Building your plan"}</Text>
            <Text style={styles.subtitle}>
              {pollTimedOut
                ? "Your plan is still generating. It’ll appear in the app shortly."
                : `Finishing ${assetName}’s plan...`}
            </Text>
          </View>
        )}
        ...
        {hasItems && heroItem ? (
          <View style={styles.tasksSection}>
            ...
            <Animated.View style={[styles.heroCard, heroStyle, { borderColor: cfg.tint + "40" }]}>
              <View style={styles.heroBadgeRow}>
                <View style={[styles.heroBadgeIcon, { backgroundColor: cfg.tint + "26" }]}>
                  <Ionicons name={cfg.icon} size={15} color={cfg.tint} />
                </View>
            ...
            <Animated.View style={[styles.bridgeBox, bridgeStyle]}>
              <Ionicons name="car-outline" size={15} color={Colors.vehicle} />
              <Text style={styles.bridgeText}>Add a vehicle next to bring your whole life into one plan.</Text>
            </Animated.View>
          </View>
        ) : !pollTimedOut ? (
          <View style={styles.tasksSection}>
        ...
        <Pressable onPress={handleAddVehicleNext} style={styles.secondary}>
          ...
          <Text style={[styles.secondaryText, { color: Colors.vehicle }]}>Add a vehicle next</Text>
        </Pressable>
