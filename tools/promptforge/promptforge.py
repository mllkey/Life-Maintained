...
- Locked escape-hatch baseline as of commit 04c00a1 (app code only, excl. supabase/functions/): as_any=82, as_unknown=4, as_typed=89, non_null=79, ts_ignore=0, ts_expect_error=0, ts_nocheck=0. Prior docs claimed as_any=89; the actual pre-Cat-3E count was 85 (grep-method discrepancy), and Cat 3-E removed 3 hatches landing at 82. This 82 baseline is canonical. Every prompt that touches app code must require a BEFORE and AFTER count and assert flat. New escape hatches require explicit founder justification AND a baseline-update step in the same commit.
- tsc ceiling: 148. Current count: 110. A prompt must require BEFORE and AFTER tsc counts and assert AFTER <= BEFORE.
...
- **Locked escape-hatch baseline.** as_any=82, as_unknown=4, as_typed=89, non_null=79, ts_ignore=0, ts_expect_error=0, ts_nocheck=0 (commit 04c00a1, app code only excluding supabase/functions/). Every STANDARD or COMPLEX prompt requires BEFORE and AFTER counts and a flat assertion. New hatches require explicit founder justification.
...
- **Every JSX edit names the exact element by first attribute or by line number.** "Wrap the FieldGroup" is ambiguous when a FieldGroup has multiple children. "Wrap the inner `<View style={{styles.field}}>` containing the Current Mileage label, NOT the outer FieldGroup" is unambiguous. Or: "Wrap the JSX element at app/add-vehicle.tsx line 412". Pick one form, never both vague and specific.
...
**13. JSX target precision.** Every prompt-specified JSX edit must name the target element by either (a) its first attribute (e.g. `<View style={styles.field}>` containing the "Current Mileage" label), (b) its exact line number, or (c) a unique grep-locatable signature. Phrases like "the FieldGroup", "the wrapping View", "the parent container" without disambiguation when multiple candidates exist = Issue. If the prompt cannot disambiguate without seeing the file, it must require Claude Code to grep/show the file first and report the candidate set before editing.
