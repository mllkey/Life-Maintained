...
as_any=82, as_unknown=4, as_typed=82, non_null=20, ts_ignore=0, ts_expect_error=0, ts_nocheck=0
Current tsc error count: 107 (ceiling: 148). Every pass must assert flat before AND after. NO new `as any`, `ts-ignore`, `ts-expect-error`, `ts-nocheck`, or `!` non-null assertions without explicit founder justification.
...
- Locked escape-hatch baseline as of commit 04c00a1 (app code only, excl. supabase/functions/): as_any=82, as_unknown=4, as_typed=82, non_null=20, ts_ignore=0, ts_expect_error=0, ts_nocheck=0. Prior docs claimed as_any=89; the actual pre-Cat-3E count was 85 (grep-method discrepancy), and Cat 3-E removed 3 hatches landing at 82. This 82 baseline is canonical. Every prompt that touches app code must require a BEFORE and AFTER count and assert flat. New escape hatches require explicit founder justification AND a baseline-update step in the same commit.
- tsc ceiling: 148. Current count: 107. A prompt must require BEFORE and AFTER tsc counts and assert AFTER <= BEFORE.
...
- **Locked escape-hatch baseline.** as_any=82, as_unknown=4, as_typed=82, non_null=20, ts_ignore=0, ts_expect_error=0, ts_nocheck=0 (commit 04c00a1, app code only excluding supabase/functions/). Every STANDARD or COMPLEX prompt requires BEFORE and AFTER counts and a flat assertion. New hatches require explicit founder justification.
