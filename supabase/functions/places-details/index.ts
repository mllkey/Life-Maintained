...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await enforceAiRateLimit(adminClient, userId, "places-details", 30, 60);
  } catch (err) {
    if (err instanceof AuthError) {
      return new Response(JSON.stringify({ addressComponents: null, error: err.message }), {
        status: err.status,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (err instanceof RateLimitError) {
      return new Response(JSON.stringify({ addressComponents: null, error: "Rate limit exceeded" }), {
        status: 429,
        headers: { ...corsHeaders, "Content-Type": "application/json", "Retry-After": String(err.retryAfterSeconds) },
      });
    }
    console.error("[places-details] auth/rate error:", err);
