-- Migration: complete_vehicle_task v2
-- Adds p_provider_name and p_did_it_myself to the maintenance_logs insert.
-- Trims return shape to the five deterministic fields callers need.
--
-- 00000 created the 7-parameter v1. Drop it explicitly before creating the
-- 9-parameter v2 so Postgres never sees two overloads with the same name,
-- which would make the bare GRANT ambiguous.

-- Drop v1 (7 params, applied by 00000)
...
-- Also drop v2 signature in case of a partial/retried apply
DROP FUNCTION IF EXISTS public.complete_vehicle_task(
  uuid,
  numeric,
  numeric,
