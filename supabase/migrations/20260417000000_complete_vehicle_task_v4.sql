-- Migration: complete_vehicle_task v4
-- Sets last_mileage_update = now() on the vehicles row when mileage increases.
-- No other behavior changes from v3.
...
BEGIN
  ...
  -- Also stamp last_mileage_update so the client-side estimator has a fresh baseline.
  ...
  -- Update vehicle hours if the new reading is higher (no last_mileage_update for hours)
